...
export default function Contacts() {
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {contact.tax_regime && <Badge variant="outline" className="text-xs mt-1">
                    {taxRegimeLabels[contact.tax_regime]}
                  </Badge>}
              </div>
            </div>
            <Badge variant={isInadimplente ? 'destructive' : 'secondary'} className={!isInadimplente ? 'bg-emerald-500/10 text-emerald-500' : ''}>
              {isInadimplente ? 'Inadimplente' : 'Adimplente'}
            </Badge>
          </div>

          <div className="space-y-2 text-sm text-muted-foreground">
            {contact.document && <p className="truncate font-mono text-xs">{contact.document}</p>}
            {contact.email && <button onClick={() => copyToClipboard(contact.email!, 'E-mail')} className="group flex items-center gap-2 w-full hover:text-primary transition-colors text-left">
            ...
            {contact.phone && <button onClick={() => copyToClipboard(contact.phone!, 'Telefone')} className="group flex items-center gap-2 w-full hover:text-primary transition-colors text-left">
                <Phone className="h-3.5 w-3.5 flex-shrink-0" />
                <span className="flex-1">{contact.phone}</span>
                <Copy className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
              </button>}
            {(contact.city || contact.state) && <div className="flex items-center gap-2">
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Gestão de Contatos</h1>
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input placeholder="Buscar por nome ou CNPJ..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 bg-background" />
            </div>
            ...
            {hasActiveFilters && <Button variant="ghost" size="sm" onClick={clearFilters} className="gap-1.5 text-muted-foreground">
                <X className="h-4 w-4" />
                Limpar Filtros
              </Button>}
          </div>
