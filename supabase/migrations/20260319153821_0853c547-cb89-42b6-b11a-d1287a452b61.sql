...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    IF OLD.bank_id IS NOT NULL THEN
      ...
      SET current_balance = initial_balance + COALESCE((
        ...
        FROM transactions 
        WHERE bank_id = OLD.bank_id AND is_paid = true AND deleted_at IS NULL AND date <= CURRENT_DATE
      ), 0)
  ...
  IF NEW.bank_id IS NOT NULL THEN
    ...
    SET current_balance = initial_balance + COALESCE((
      SELECT SUM(CASE WHEN type = 'receita' THEN COALESCE(paid_amount, amount) ELSE -COALESCE(paid_amount, amount) END)
      FROM transactions 
      WHERE bank_id = NEW.bank_id AND is_paid = true AND deleted_at IS NULL AND date <= CURRENT_DATE
    ), 0)
